...
// Made by KuroiSH - Securitygab
	for _, v := range scanners {
		if len(v.matches) == 0 {
			continue
		}
		if v.provider == "google_access_token" {
			fmt.Println("secret-key > Found Google access tokens:")
			...
			fmt.Println("secret-key > Found Google API keys:")
			for _, m := range v.matches {
				fmt.Printf("- %s\n", m)
				if testFlag {
			...
			fmt.Println("secret-key > Found Slack keys:")
			for _, m := range v.matches {
				fmt.Printf("- %s\n", m)
				if testFlag {
			...
			for _, m := range v.matches {
                ...
                if testFlag { 
					c, err :=redis.DialURL(m, redis.DialConnectTimeout(time.Second))
				     if err == nil { 
						 fmt.Print("secret-key > Connection succesful!\n\n")
					    defer c.Close()
						}
				}
					} else {
